...
const JourneyCard: React.FC<JourneyCardProps> = ({
  ...
  const categoryColors: Record<JourneyStep['category'], string> = useMemo(() => ({
    Introduction: "bg-linear-to-r from-blue-500/20 to-indigo-500/20 text-blue-200 border border-blue-400/30",
    Education: "bg-linear-to-r from-emerald-500/20 to-teal-500/20 text-emerald-200 border border-emerald-400/30",
    Development: "bg-linear-to-r from-orange-500/20 to-amber-500/20 text-orange-200 border border-orange-400/30",
    Experience: "bg-linear-to-r from-purple-500/20 to-violet-500/20 text-purple-200 border border-purple-400/30",
    Expertise: "bg-linear-to-r from-pink-500/20 to-rose-500/20 text-pink-200 border border-pink-400/30"
  }), []);
  ...
  const renderProgressSteps = () => {
    ...
    steps.push(
      <div key="progress-line" className="flex-1 flex items-center relative">
        ...
        <div
          className="h-1 bg-linear-to-r from-blue-400 to-purple-400 absolute transition-all duration-300 rounded-full"
          style={{ width: progressWidth }}
        ></div>
        {/* Add the smaller current position indicator */}
        <div
          className="w-5 h-5 rounded-full border border-blue-400 bg-linear-to-br from-blue-900 to-purple-900 absolute z-20 shadow-xs transition-all duration-300"
        ...
        {Array.from({ length: totalSteps }).map((_, index) => (
          <div 
            ...
            className={`w-2.5 h-2.5 rounded-full absolute z-10 cursor-pointer transition-all duration-200 hover:scale-110 ${
              index + 1 <= activeStep ? 'bg-blue-400 shadow-xs shadow-blue-500/30' : 'bg-blue-500/30 hover:bg-blue-400/60'
            }`}
  ...
  return (
    <div 
      className="w-full max-w-none sm:max-w-[800px] focus:outline-hidden mx-auto"
      tabIndex={0}
      onKeyDown={handleKeyNavigation}
      role="region"
      aria-label="Journey timeline"
    >
      <div className="w-full bg-linear-to-br from-gray-900/60 to-black/80 border border-blue-500/20 rounded-2xl shadow-xl backdrop-blur-xs p-3 sm:p-4 md:p-6 transition-all duration-300 relative overflow-hidden group">
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-center border-b border-blue-500/20 pb-3 sm:pb-4 mb-4 sm:mb-6">
            <h1 className="text-xl sm:text-2xl font-bold text-white bg-linear-to-r from-white to-blue-100 bg-clip-text">Journey</h1>
          ...
          <div className={`border border-blue-400/20 rounded-xl p-4 sm:p-6 bg-linear-to-br from-blue-900/20 to-purple-900/20 shadow-lg transition-all duration-200 backdrop-blur-xs ${isAnimating ? 'opacity-75' : 'opacity-100'}`}>
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 sm:mb-6 gap-3 sm:gap-4">
              ...
              <div className="flex gap-4 items-start">
                <div
                  className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-200 ${isAnimating ? 'opacity-0 scale-95' : 'opacity-100 scale-100'} ${categoryColors[category] || "bg-blue-500/20 text-blue-200 border border-blue-400/30"} backdrop-blur-xs`}
                >
                  {category}
                </div>
              </div>
                  ...
                  <ul className="space-y-2 sm:space-y-3">
                    {currentStepData.highlights.map((highlight, index) => (
                      <li key={index} className="flex items-start">
                        <span className="inline-block w-1.5 sm:w-2 h-1.5 sm:h-2 rounded-full bg-blue-400 mt-2 mr-2 sm:mr-3 shrink-0" />
