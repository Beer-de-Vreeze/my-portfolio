...
const TechItem: React.FC<TechItemProps> = React.memo(({ icon, name }) => {
  return (
    <div 
      className="flex flex-col items-center justify-center p-3 sm:p-4 bg-linear-to-br from-blue-900/20 to-purple-900/20 border border-blue-400/20 rounded-xl shadow-lg transition-all duration-300 hover:scale-105 hover:border-blue-300/40 hover:shadow-blue-500/20 hover:shadow-xl group backdrop-blur-xs min-h-[100px] sm:min-h-[120px]"
      role="button"
      tabIndex={0}
    ...
    >
      ...
      <div className="text-blue-100 text-xs sm:text-sm font-medium text-center w-full wrap-break-word hyphens-auto transition-colors duration-300 group-hover:text-white leading-tight">
        {name}
      </div>
    </div>
  );
...
const SkillCard: React.FC<TechStackProps> = ({ title = "Tech Stack" }) => {
  ...
  return (
    <div className="w-full max-w-none sm:max-w-[800px] bg-linear-to-br from-gray-900/60 to-black/80 border border-blue-500/20 rounded-2xl shadow-xl backdrop-blur-xs p-3 sm:p-4 md:p-6 relative overflow-hidden group mx-auto">
      ...
      <div className="relative z-10 w-full">
        <div className="flex justify-between items-center border-b border-blue-500/20 pb-3 sm:pb-4 mb-4 sm:mb-6">
          <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-white bg-linear-to-r from-white to-blue-100 bg-clip-text">{title}</h1>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mb-6 sm:grid-cols-3 md:flex md:flex-wrap md:gap-3">
          {categories.map((category) => (
            <button
              ...
              } backdrop-blur-xs active:scale-95 transform text-center`}
              type="button"
            ...
            >
              ...
              {category.active && (
                <div className="absolute inset-0 bg-linear-to-r from-blue-500/10 to-purple-500/10 rounded-xl" />
              )}
            </button>
          ))}
        </div>
